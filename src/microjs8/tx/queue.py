...
def infer_outbound_kind(text: str) -> OutboundKind:
    ...
    # Phase 12: @-PREFIXED RECIPIENT → fire-and-forget regardless of
    # verb. JS8Call groups (@EMCOMM, @ARESGA, @SKYWARN) and the
    # universal broadcasts (@ALLCALL, @HB) all share one property:
    # there is no single recipient station whose ACK closes the loop.
    # A group MSG sent to @EMCOMM is heard and ACK'd by EVERY
    # @EMCOMM member, which means the WAIT_ACK queue would never see
    # "its" expected ACK callsign and would retransmit the MSG until
    # attempts exhaust — exactly the W5DMH bench symptom on the May
    # 2026 build: `@EMCOMM MSG <body>` got retransmitted 2-3 times
    # even though multiple stations had already inboxed it and ACK'd.
    # Per the operator's spec, group MSG should be fire-and-forget:
    # the operator can watch the DIRECTED log to see which stations
    ...
    #
    # The explicit OutboundKind.HEARTBEAT / CQ / ALLCALL kinds set
    # by those code paths still win over this inference — this is
    # the fallback path used by ``enqueue_for_encoding`` when no
    # kind is supplied (the compose-send path that lands in Phase 14).
...
class OutboundQueue:
    ...
    def purge_stale_broadcasts(self) -> dict[str, int]:
        ...
        QUEUED, SENDING, WAIT_ACK). Already-DELIVERED rows are kept
        for the outgoing-message log. WAIT_ACK doesn't apply to
        broadcasts in practice (no single station's ACK closes a
        broadcast) but we sweep it for safety.
        """

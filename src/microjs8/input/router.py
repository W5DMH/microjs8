...
class InputRouter:
    ...
    def _handle_screen_key(self, event: KeyEvent, snapshot) -> None:
        ...
        # v0.0.10: Directed activity log scrolling.
        # ↑ scrolls toward newer (offset −), ↓ scrolls toward older
        # (offset +). The renderer clamps to the visible window and
        # the state methods refuse to scroll past either end. ←/→
        # still cycle screens (fall through to ring nav).
        if snapshot.screen is Screen.DIRECTED:
            if event.key is Key.UP:
                self._ui.directed_scroll_up()
                return
            if event.key is Key.DOWN:
                self._ui.directed_scroll_down()
                return
            if event.key is Key.ENTER:
                # Enter is reserved for future detail-view; no-op for now.
                return

        # v0.0.10: Heard list scrolling — same ↑/↓ semantics as
        # DIRECTED. Operators with many heard stations couldn't
        # access the older ones (8 visible rows max) before this
        # was added. Now ↓ walks down the full heard table.
        if snapshot.screen is Screen.HEARD:
            if event.key is Key.UP:
                self._ui.heard_scroll_up()
                return
            if event.key is Key.DOWN:
                self._ui.heard_scroll_down()
                return
            if event.key is Key.ENTER:
                return
    ...
    def _handle_allcall_key(self, event: KeyEvent, snapshot) -> bool:
        ...
        if event.key is Key.ENTER:
            ...
            if focus == 0:
                ...
                # The DIRECTED jump for HB happens after MODE COMMIT
                # in _handle_hb_mode_select_key, not here.
                ...
                if self._allcall_query_msgs is not None:
                    ...
                    # v0.0.10: jump to DIRECTED so the operator sees
                    # the outgoing broadcast appear in the activity
                    # log (logged by app.py's allcall_query_msgs
                    # implementation). Jumping even on a failed fire
                    # is OK — the log would be empty but the screen
                    # change makes "I did something" feedback obvious.
                ...
                if self._allcall_cq is not None:
                    ...
                    # v0.0.10: jump to DIRECTED (see QUERY MSGS rationale)
                    self._ui.set_screen(Screen.DIRECTED)
            return True
        return False
    ...
    def _handle_hb_mode_select_key(
        ...
        ↑/↓ cycle focus, Enter commits + returns to ALLCALL (then
        on to DIRECTED if a beacon-firing mode was chosen), Esc
        cancels + returns to ALLCALL. Returns True if consumed.

        v0.0.10: after committing a beacon-firing mode (SINGLE / 20MIN
        / 1HR), the operator transitions to DIRECTED so they see the
        first heartbeat TX appear in the activity log. Committing OFF
        returns to ALLCALL — no log entry is generated by stopping a
        beacon, so the DIRECTED screen has nothing new to show.
        """
        if event.key is Key.UP:
        ...
        if event.key is Key.DOWN:
            ...
            return True
        if event.key is Key.ENTER:
            # Snapshot the focused mode BEFORE commit (close_hb_mode_select
            # may reset focus state) so we know whether to navigate.
            from microjs8.ui.state import HB_MODES_ORDERED, HbMode
            focused_idx = snapshot.hb_select_focus
            new_mode = (
                HB_MODES_ORDERED[focused_idx]
                if 0 <= focused_idx < len(HB_MODES_ORDERED)
                else HbMode.OFF
            )
            self._ui.close_hb_mode_select(commit=True)
            # v0.0.10: if the operator chose a firing mode, jump to
            # DIRECTED so they see the beacon TX appear in the log.
            # OFF returns to ALLCALL (nothing new will appear in
            # DIRECTED — committing OFF doesn't fire anything).
            if new_mode is not HbMode.OFF:
                self._ui.set_screen(Screen.DIRECTED)
            return True
        if event.key is Key.ESC:
